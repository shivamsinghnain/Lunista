...
float getDensity(vec3 rayPos) {
    ...
    float baseDensityFBM = alligatorNoise.g * 0.5 + alligatorNoise.b * 0.3 + alligatorNoise.a * 0.2; 
    float baseDensity = remap2(alligatorNoise.x, baseDensityFBM - 1.0, 1.0);

    vec4 perlinNoise = textureLod(perlinNoiseTex, rayPos.xz / CLOUD_3D_NOISE_TEXTURE_SIZE_M, 0.0);
    vec4 worleyNoise = textureLod(worleyNoiseTex, rayPos.xz / CLOUD_3D_NOISE_TEXTURE_SIZE_M, 0.0);
...
vec4 raymarch(vec3 rayOrigin, vec3 rayDirection, vec3 planetLightColor) {
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        if (sampleDensity > 1e-6) {
            ...
            float transmittanceAtPoint = beersLaw(sampleDensity * (length(raySteps)), extinctionCoefficient);
