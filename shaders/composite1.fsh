...
uniform sampler3D colortex10; //Alligator Noise
...
vec4 raymarch(vec3 rayOrigin, vec3 rayDirection, vec3 planetLightColor) {
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        vec3 rayPos = startPos + rayDirection * float(i);

        float sampleDensity = getCloud(rayPos / 1024.0);

        if (sampleDensity > 1e-6) {

            vec3 scatteredLight = planetLightColor * 1.0;
...
void main() {
    ...
    // vec4 noise = textureLod(colortex10, playerFeetPos / CLOUD_3D_NOISE_TEXTURE_SIZE_M);
