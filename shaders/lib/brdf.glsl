...
vec3 computeBRDF(in vec3 N, in vec3 V, in vec3 L, in vec3 albedo, in float reflectance, in float roughness, in vec3 directLight) {
    ...
    float NoH = clamp(dot(N, H), 1e-6, 1.0);
    float NoL = clamp(dot(N, L), 1e-6, 1.0);
    float NoV = clamp(dot(N, V), 1e-6, 1.0);
    float VoL = dot(V, L);

    float NoH2 = (getNoHSquared(NoL, NoV, VoL, 0.01));
    ...
    float NDF = distributionGGX(roughness, NoH2);
